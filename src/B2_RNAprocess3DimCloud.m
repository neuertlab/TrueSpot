...
for kk=1:Z;
    for nn = 1:f;
       ...
       % figure(1); clf; imshow(TMR1,[])
        TMR2 = imfilter(TMR1,w2); % spot filtering
        %figure(1); clf; imshow(TMR2,[0 15])
        TMR3 = TMR2 > th1; % remove pixels below th1
        %figure(1); clf; imshow(TMR3,[0 1])
        TMR4 = bwareaopen(TMR3,p,con); % remove objects that are smaller than p pixels and count connected regions with con pixels as one object
        %figure(2); clf; imshow(TMR4,[0 1])
...
for kk=1:Z;
 ...
 TMR3Dback(:,:,kk) = medfilt2(TMR, [100 100]); % gauss smoothing
   % figure(1); clf; imshow(TMR3Dback(:,:,kk),[])
