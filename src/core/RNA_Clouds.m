    ...
    methods
        ...
        function [cloud_mask, cloud_boxes] = detectClouds3D(obj, img, nuc_coord_table, z_adj)
            ...
            for s = 1:spotcount
                ...
                cloud_box = NaN(1,6);
                cloud_box(1,1) = floor(min(cloud_box_all(:,1)));
                cloud_box(1,2) = floor(min(cloud_box_all(:,2)));
                cloud_box(1,3) = floor(min(cloud_box_all(:,3)));
                cloud_box(1,4) = ceil(max(cloud_box_all(:,4))) - cloud_box(1,1);
                cloud_box(1,5) = ceil(max(cloud_box_all(:,5))) - cloud_box(1,2);
                cloud_box(1,6) = ceil(max(cloud_box_all(:,6))) - cloud_box(1,3);
