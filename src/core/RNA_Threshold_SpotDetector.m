...
classdef RNA_Threshold_SpotDetector
    ...
    methods (Static)
        ...
        %           processing progress
        %
        function [auto_ztrim, new_th_min] = run_spot_detection_main(IMG_filtered, save_stem, strategy, th_min, th_max, ztrim, limit2gb, verbose, thread_request)
            if nargin < 6
                ztrim = 0;
            end
            if nargin < 7
...
%             auto_ztrim = z_trim;
            ...
            T = size(spot_table,1);
            [spot_table, coord_table] = RNASpotsRun.saveTables(spot_table, coord_table, save_stem, limit2gb);
