function B_ClusterImageProcessingTMR_AF594_CY5_B(Task_Number,maxSample,maxIm,Ych,Ypos,globalpath,exppath,osmo,strain,genepair,exp_names,filepath1,...
...
if exist(files, 'file') == 0
    ...
    %% Load segmented cells',ch,'_im',num2str(m),'.mat'
      ...
      if Ywin == 0;
            fileLAB = char(strcat(filepath1, '/Lab_',exp_name_1,'_',num2str(Im1),'.mat'))
        else;
            fileLAB = char(strcat(filepath1, '\Lab_',exp_name_1,'_',num2str(Im1),'.mat'))
        end;  
    ...
    %% Load 3D nucleus    
        if Ywin == 0;
            fileNUC = char(strcat(filepath1, '/nuclei_',exp_name_1,'_',num2str(Im1),'.mat'))
        else;
            fileNUC = char(strcat(filepath1, '\nuclei_',exp_name_1,'_',num2str(Im1),'.mat'))
        end;
        %load(fileLAB,'Lab','Nuc3D'); %'NucInfo','CellInfo',
...
%         Nuc3D = nuclei;
        ...
        %         figure; imshow(AF594maxF,[0 1]);
            ...
            if Ych(5) == 1;
                ...
                [TMRmax,TMRmaxF,thallTMR,TMR3D3immax,TMRmaxFimmax,TMR3Dorg,TMR3Dfilter,TMR3Dback]...
                    = B2_RNAprocess3Dim(TMR_ims,thTMR,f);
  ...
  tic
        ...
        if Ych(3) == 1
            'Low threshold nuc'
            Nuc3D = Label_low;
            [CELLmaxRNAcy5_low,RNAposCY5,tran_cloud,clouds1,clouds2]...
                =B3_nongaussRNAposCloud(Lab,mm,CY53Dorg,CY53Dback,CY53D3immax,CY53Dfilter,Nuc3D);
            'Mid threshold nuc'
            Nuc3D = Label_mid;
            [CELLmaxRNAcy5_mid,RNAposCY5,tran_cloud,clouds1,clouds2]...
                =B3_nongaussRNAposCloud(Lab,mm,CY53Dorg,CY53Dback,CY53D3immax,CY53Dfilter,Nuc3D);
            'Hi threshold nuc'
            Nuc3D = Label_hi;
            [CELLmaxRNAcy5_hi, RNAposCY5,tran_cloud,clouds1,clouds2]...
                =B3_nongaussRNAposCloud(Lab,mm,CY53Dorg,CY53Dback,CY53D3immax,CY53Dfilter,Nuc3D);
        else;
        ...
        if Ych(3) == 1 & Ypos == 1
            Nuc3D = Label_mid;
            [PARcy5_mid] = B4_FitGaussians3Cloud(Lab, mm,CY53Dorg,CY53Dback,CY53D3immax,CY53Dfilter,RNAposCY5,Nuc3D,tran_cloud,clouds1,clouds2);
             Nuc3D = Label_low;
        ...
        if Ych(5) == 1
            'Low threshold nuc'
            Nuc3D = Label_low;
            [CELLmaxRNAtmr_low,RNAposTMR]...
                =B3_nongaussRNApos(Lab,mm,TMR3Dorg,TMR3Dback,TMR3D3immax,TMR3Dfilter,Nuc3D);
            'Mid threshold nuc'
            Nuc3D = Label_mid;
            [CELLmaxRNAtmr_mid,RNAposTMR]...
                =B3_nongaussRNApos(Lab,mm,TMR3Dorg,TMR3Dback,TMR3D3immax,TMR3Dfilter,Nuc3D);
            'Hi threshold nuc'
            Nuc3D = Label_hi;
            [CELLmaxRNAtmr_hi, RNAposTMR]...
                =B3_nongaussRNApos(Lab,mm,TMR3Dorg,TMR3Dback,TMR3D3immax,TMR3Dfilter,Nuc3D);
        else;
        ...
        if Ych(5) == 1 & Ypos == 1
            Nuc3D = Label_mid;
            [PARtmr_mid] = B4_FitGaussians3(Lab, mm,TMR3Dorg,TMR3Dback,TMR3D3immax,TMR3Dfilter,RNAposTMR,Nuc3D);
             Nuc3D = Label_low;
