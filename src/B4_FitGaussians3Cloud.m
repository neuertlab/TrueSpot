function [PAR]...
    = B4_FitGaussians3Cloud(Lab, mm,TMR3Dorg,TMR3Dback,TMR3D3immax,TMR3Dfilter,RNAposTMR,Nuc3D,tran_cloud,clouds1,clouds2)  
%RNAposTMR = RNApos;
d = 4; %Defines the image size to be 4 pixels greater than nucleus boundary
% generate masks
b = double(ones(d*2+1,d*2+1)); 
b(2:d*2,2:d*2) = 0; 
b1= (1-b)*0.5;
b2 = b1;
b2(3:d*2-1,3:d*2-1) = 2;
b2 = b2 ./2;
...
PAR.TotExpIntCloud1 = NaN(mm,nmbr); %Integrated Pixel Intensity for experiment (with background subtracted out) with the cloud mask
PAR.TotExpIntCloud2 = NaN(mm,nmbr); %Integrated Pixel Intensity for experiment (with background subtracted out) with the cloud mask
PAR.TotExpIntMultSlic = NaN(mm,nmbr); %Integrated Pixel Intensity for experiment (with background subtracted out)
...
clouds1 = uint16(clouds1);
clouds2 = uint16(clouds2);  
PAR.clouds1 = clouds1;              %Image with all first clouds
PAR.clouds2 = clouds2;              %Image with all second clouds
     
        for q = 1:mm
            %q = 29;
            len = nnz(RNAposTMR(q,:,1));  %the number of RNA spots in each cell
            
            clear k1 k2 Nuc Cyto NucTMR2d CellTMR1g CellTMR2d L CellTMR2dorg NucTMR2dorg CellRNA CellRNA2 CellRNAorg Nuc Cyto Cell CytoRNA2 NucRNA2 rLabeltcell NUMcell1 rLabeltcyto NUMcyto1 rLabeltnuc NUMnuc1 CellCloud CellCloud1 CellCloud2;
            ...
            i = 1;
            clear CellRNAfilter CellRNAindex CellRNAorg CellRNAback Nuc Cyto Cell CellCloud
            ...
            for i=1:zz;
                ...
                CellCloud1(:,:,i) = immultiply(clouds1(Y0:Y1,X0:X1,i),k2); % Cloud mask inside the cell
                CellCloud2(:,:,i) = immultiply(clouds2(Y0:Y1,X0:X1,i),k2); % Cloud mask inside the cell
...
%             [rLabeltnuc,NUMnuc1] = bwlabeln(NucRNA2 > 0,26); 
            ...
            if len ~= 0 %only fit if there is an RNA spot
                for cnt = 1:len
                    clear par5 TMRb TMRimm2 TMRint
                    if cnt > 2 || (cnt == 2 & tran_cloud(2,q) == 0); %If it's the third RNA (first two are always clouds if possible) or there is no second cloud
                        d = 4; %Defines the image size to be 4 pixels greater than nucleus boundary
                        % generate masks
                        b = double(ones(d*2+1,d*2+1));
                        b(2:d*2,2:d*2) = 0;
                        ...
                        if k < 2;
                            k3 = k-1;           %how much below z position of RNA to look
                        else;
                            k3 = 2;
                        end;
                        if k > zz - 2;
                            k4 = 1;             %how much above z position of RNA to look
                        else;
                            k4 = 2;
                        end;
                    ...
                    elseif cnt < 3 %same as above, but now dependent on d
                        d = tran_cloud(cnt,q) %Defines the image size to be 4 pixels greater than nucleus boundary
                        % generate masks
                        b = double(ones(d*2+1,d*2+1));
                        b(2:d*2,2:d*2) = 0;
                        b1= (1-b)*0.5;
                        ...
                        %% set lower and upper limits in the stack
                        if k <= d/2;
                            k3 = k-1;           %how much below z position of RNA to look. looks at 1 of k is close to bottom
                        else;
                            k3 = d/2;
                        end;
                        if k > zz - d/2
                            k4 = zz-k;             %how much above z position of RNA to look
                        else;
                            k4 = d/2;
                        end;
                        %% include limits for x and y direction
                        ...
                        %                   xA = yall(xynan(i),k);
                        counter76 = 1;
                        stacks = (k-k3):(k+k4);     %which stacks to look at
                        for ij = stacks
                            TMRfree(:,:,counter76) = CellRNAorg2(:,:,ij) - Cellback2(:,:,ij);
                        ...
                        if endx > size(TMRfree,1)
                            endx = size(TMRfree,1)
                        end
                         if endy > size(TMRfree,2)
                             endy = size(TMRfree,2)
                         end
                         CellCloud1A = zeros(endx-begx+1,endy-begy+1,size(TMRfree,3));
                         CellCloud2A = zeros(endx-begx+1,endy-begy+1,size(TMRfree,3));
                         'CellCloud1A' 
                         size(CellCloud1A)
                         'CellCloud1'
                         size(CellCloud1)
                         ...
                         'TMRfree'
                         size(TMRfree)
                        for ij = 1:size(TMRfree,3)
                            TMRim1(:,:,ij) = TMRfree(begx:endx,begy:endy,ij); % generate image for single pixcel 1:9 x 1:9
                            CellCloud1A(:,:,ij) = CellCloud1(begx:endx,begy:endy,stacks(ij)); %cloud mask for RNA viewing window
                            CellCloud2A(:,:,ij) = CellCloud2(begx:endx,begy:endy,stacks(ij)); %cloud mask for RNA viewing window
                        end
                        %Below was added 7/10/17 to avoid an error with
                        %different sizes
                        if size(TMRim1,1) ~= size(b,1) | size(TMRim1,2) ~= size(b,2)
                            b = double(ones(size(TMRim1(:,:,1))));
                            b(2:size(b,1)-1,2:size(b,2)-1) = 0;
                            b1= (1-b)*0.5;
                            b2 = b1;
                            b2(3:size(b,1)-2,3:size(b,2)-2) = 2;
                            b2 = b2 ./2;   
                        end
                        % End of 7/10/17 addition
                        %                 logTMRfree1 = TMRfree(:,:,3) >= 0;
                        %                 logTMRfree2 = log(immultiply(TMRfree(:,:,3),logTMRfree1) + 1);
                        %     figure(105); clf; subplot(1,2,1); imshow(TMRfree(:,:,3),[]); hold on; plot(yA,xA,'o'); hold on; title(['Cell: ' num2str(q) ', Plane: ' num2str(k)]); ...
                        %     subplot(1,2,2); imshow(TMRim1(:,:,3),[]); title(['RNA: ' num2str(i) ', maxRNA: ' num2str(size(xynan,1))]); impixelinfo;
...
%     subplot(1,2,2); imshow(max(TMRim,[],3),[]); title(['RNA: ' num2str(i)  ', maxRNA: ' num2str(size(xynan,1))]); impixelinfo;
                    ...
                    TMRint(1:size(TMRim,3),1) = sum(sum(TMRim,1),2);  % Determine total fluorecnes under the peak  %CHANGED FROM ":" to "1:size(TMRim,3)" BK
        %            [X1,Y1] = meshgrid(1:1:2*d+1);
                    if cnt > 2 || (cnt == 2 & (max(clouds2(:)) == 0))
                        ...
                        par0(1) = d+1; % initial condition g1 x-position
                        par0(2) = d+1; % initial condition g1 y-position
                        par0(3) = d-4; % initial condition s1 width of gaussian
                    end
                    double(max(TMRim(:)))
                    par0
                    ...
                    mp2 = size(TMRim,2);
                    [X1,Y1] = meshgrid(1:1:mp2,1:1:mp);
%                     figure(201); imshow(X1,[])
%                     figure(202); imshow(Y1,[])
                    
                    options = optimset('Display', 'none','MaxFunEvals', 200,'MaxIter', 200,'TolX', 1E-9); % fit conditions
                    jj = 10;
                    %In the below code, I changed a lot of parameters
                    %previously set to 3 to be the plane that has the
                    %brightest pixel.
                    for jj = 1:size(TMRfree,3);                                 %fits a Gaussian to each slice
                        TMRimm = double(TMRim(:,:,jj));% Spot image
                        TMRimm2(1:size(TMRimm,1),1:size(TMRimm,2),jj) = TMRimm;
                        TMRimm1 = TMRimm./max(TMRimm(:));   %normalized to maximum 
                        ...
                        try r(jj) = IR.MajorAxisLength ./ IR.MinorAxisLength; % deterine ratio of the two axis => circular = 1 single spot; >1.3 multiple spots
                        lb = [2 2 par0(3) par0(4)]; % lower bounds for finding the position in x and y
                        ub = [mp-2 mp-2  par0(3) 5*par0(4)] + 0.001; end;% upper bounds for finding the position in x and y
                        try [par,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(@Gauss3DFit,par0,mp,TMRimm,lb,ub,options);% least square fit for the position      
                        ...
                        try [par2,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(@Gauss3DFit,par0,mp,TMRimm,lb,ub,options); % least square fit for the gaussian width and amplitude  
                        ...
                        par3([1 2 3 5]) = par2;
                        par3(4) = par2(3);
                        try [par4,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(@Gauss3DFitS1S2,par3,mp,TMRimm,lb,ub,options); % least square fit for the gaussian width in x and y and amplitude  
                        catch
                        par4(1:5) = NaN;
                        ['Gaussian fitting failed for cell ' num2str(q) ' RNA number ' num2str(cnt)]
                        end
                        g3 = par4(5)*exp(-((X1-par4(1)).^2)./(2*par4(3)^2)-((Y1-par4(2)).^2)./(2*par4(4)^2)); % compute gaussain after fit
                        gAll(1:size(TMRimm,1),1:size(TMRimm,2),jj) = g3;
                        par4;
                    ...
                    midp = round(size(TMRfree,3)/2);
                    try PAR.xfit(q,cnt) = par5(1,midp); end; % fitted x-parameter
                    try PAR.yfit(q,cnt) = par5(2,midp); end; % fitted y-parameter
                    try PAR.xinit(q,cnt) = xA; end; %initial x-position
                    try PAR.yinit(q,cnt) = yA; end; % initial; y-position
                    try PAR.xgw(q,cnt) = par5(3,midp); end; % fitted gaussian width x
                    try PAR.ygw(q,cnt) = par5(4,midp); end; % fitted gaussian width y
                    try PAR.xFWHM(q,cnt) = 2*sqrt(2*log(2)).*par5(3,midp); end; % fitted FWHM x
                    try PAR.yFWHM(q,cnt) = 2*sqrt(2*log(2)).*par5(4,midp); end; % fitted FWHM y
                    try PAR.expMInt(q,cnt) = double(max(TMRimm2(:))); end;  % experimental maximum spot intensity
                    try PAR.fitMInt(q,cnt) = par5(5,midp); end; % fitted maximum spot intensity
                    try PAR.TotExpInt(q,cnt) = nansum(nansum(TMRimm2(:,:,midp))); end; % experimental integrated spot intensity
                    try PAR.TotExpIntMultSlic(q,cnt) = nansum(nansum(TMRimm2(:))); end; %Integrated Pixel Intensity for experiment (does not seem to have background subtracted out)
                    if cnt ==1
                        try 
                            CellCloud1A = double(CellCloud1A);
                            cloudFluor = immultiply(TMRimm2,CellCloud1A);
                            PAR.TotExpIntCloud1(q,cnt) = nansum(cloudFluor(:)); %Integrated Pixel Intensity for experiment (does not seem to have background subtracted out) with the cloud mask
                        catch
                            ['Integrated intensity across multiple slices failed for cell ' num2str(q) ' RNA number ' num2str(cnt)]
                        end
                    elseif cnt ==2 & tran_cloud(2,q) > 0
                        try 
                            CellCloud2A = double(CellCloud2A);
                            cloudFluor = immultiply(TMRimm2,CellCloud2A);
                            PAR.TotExpIntCloud2(q,cnt) = nansum(cloudFluor(:)); %Integrated Pixel Intensity for experiment (does not seem to have background subtracted out) with the cloud mask
                        catch
                            ['Integrated intensity across multiple slices failed for cell ' num2str(q) ' RNA number ' num2str(cnt)]
                        end                        
                    else
                        try 
                            PAR.TotExpIntMultSlic(q,cnt) = nansum(TMRimm2(:)); %Integrated Pixel Intensity for experiment (does not seem to have background subtracted out) with the cloud mask
                        catch
                            ['Integrated intensity across multiple slices failed for cell ' num2str(q) ' RNA number ' num2str(cnt)]
                        end
                    end
                    try PAR.TotFitInt(q,cnt) = nansum(nansum(gAll(:,:,midp))); end; % fitted integrated spot intensity
...
%                        plot(ydata2,'or'); hold on; plot(z1,df2,'r');
                    ...
                    [s,I] = max(ydata3);
                    if (I > 2 && I < size(TMRfree,3));
                        p3 = polyfit([1:size(TMRfree,3)],ydata3,4);
                        x2 = 1:0.01:size(TMRfree,3);
