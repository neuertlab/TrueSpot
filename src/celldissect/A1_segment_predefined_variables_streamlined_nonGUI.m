...
function A1_segment_predefined_variables(Yth,Ych,ManTh,Ywin,Yim,file_type,segment_mode,Seg_thres,fish_dir,outfile_prefix_seg,min_nucleus_size,max_nucleus_size,min_cell_size,max_cell_size,img_stacks,yeast_seg,images1,im_prefixes,dxy1)
% matlabpool
...
%         end;
    ...
    if false;%exist([outfile_prefix_seg fileTH],'file'); % test if the file exist allready     
       ...
       if Seg_thres == 1
           [dapi_threshold,dapi_label,counter2,max_nuclei] = B1_autosegment_nuclei8_thres(DAPI_ims,Yim,ManTh,task_id,counter2,max_nuclei,counter,exp_date{task_id},min_nucleus_size,max_nucleus_size);  
       else
   ...
   % save(strcat(outfile_prefix_seg, 'max_nuclei', exp_date{task_id}), 'max_nuclei')
    ...
    if false;%exist([outfile_prefix_seg fileDAPI],'file');
       ...
       if yeast_seg
         [Label_low,Label_mid,Label_hi,nuclei,Nuc_int,Nuc_vol,Maj_axis,Min_axis] = B1_autosegment_nuclei5_yeast(DAPI_ims,dapi_label,Yim,dapi_threshold,min_nucleus_size,max_nucleus_size); % Segment DAPI images in 3D with three different thresholds
       save([outfile_prefix_seg fileDAPI],'Label_low','Label_mid','Label_hi','nuclei','Nuc_int','Nuc_vol','Maj_axis','Min_axis','-mat','-v7.3');     % Save the segmented nuclei      
        ...
        [Label_low,Label_mid,Label_hi,nuclei,Nuc_int,Nuc_vol,Maj_axis,Min_axis,dapi_label] = B1_autosegment_nuclei5(DAPI_ims,dapi_label,Yim,dapi_threshold,min_nucleus_size,max_nucleus_size,dxy1,dapi_label_low1); % Segment DAPI images in 3D with three different thresholds
       save([outfile_prefix_seg fileDAPI],'Label_low','Label_mid','Label_hi','nuclei','Nuc_int','Nuc_vol','Maj_axis','Min_axis','-mat','-v7.3');     % Save the segmented nuclei      
       end
     end;
