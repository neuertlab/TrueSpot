...
Seg = 0;                                                                    %Set to 1 if you want to threshold the images (must be done before RNA)
Yth = 0;                                                                     %set to 1 if just want thresholding and not segmentation
RNA_thres = 1;                                                              %Set to 1 if you want to determine RNA spots (must be done after segmentation)
...
img_stacks = [45,70];                                                        %specify the image stacks used for segmentation. These are number of stacks above the plane with least contrast (in focus)
...
exp_date = '20171003';                                                        % Date of the experiment and folder name on the hard disk
strain = 'mESC_2i-serum-adh';
osmo = '0.2MStep';
...
%% Run the segmentation or RNA threshold determination
 ...
 if RNA_thres == 1
     Run_RNA_detection_predefined_variables(thA,ths,outfile_prefix_RNA,Yth,Ych,ManTh,Ywin,Yim,task_ids,file_type,exp_date,strain,osmo,genepair,file_end,exp_names,positions_touse,segment_mode,Seg_thres,fish_dir,outfile_prefix_seg,img_num,node_num,max_int_thres,max_int_spot_th)
 end   
