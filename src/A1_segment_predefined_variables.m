...
function A1_segment_predefined_variables(Yth,Ych,ManTh,Ywin,Yim,task_ids,file_type,exp_date,strain,osmo,genepair,file_end,exp_names,positions_touse,segment_mode,Seg_thres,fish_dir,outfile_prefix_seg,min_nucleus_size,max_nucleus_size,min_cell_size,max_cell_size,img_num,img_stacks)
% matlabpool
...
for task_id = task_ids;                                                            % Specify the job (experiment and time point) that needs to be processes. comment out if run in a loop or on the cluster
%task_id = 1
...
for counter=img_num
    ...
    exp_name = [exp_date '_' exp_names{task_id} '_'  strain   '_' genepair '_img'];
    ...
    counter
    fileTH = ['nuclei_TH_' exp_name  num2str(counter) '.mat']
    if exist([outfile_prefix_seg fileTH],'file'); % test if the file exist allready     
       load([outfile_prefix_seg fileTH])   % load the existing file
    else                                                                    % if the file does not exist, process DAPI images
       if Seg_thres == 1
           [dapi_threshold,dapi_label,counter2,max_nuclei] = B1_autosegment_nuclei8_thres(DAPI_ims,Yim,ManTh,task_id,counter2,max_nuclei,counter,exp_date,min_nucleus_size,max_nucleus_size);  
       else
           [dapi_threshold,dapi_label,counter2,max_nuclei,DAPI_ims_added] = B1_autosegment_nuclei8_adding_indiv(DAPI_ims,Yim,ManTh,task_id,counter2,max_nuclei,counter,exp_date,min_nucleus_size,max_nucleus_size,Ywin);
       end
       save([outfile_prefix_seg fileTH],'dapi_threshold','dapi_label','DAPI_ims_added','-mat');     % Save the segmented nuclei
    end;
    save(strcat(outfile_prefix_seg, 'max_nuclei', exp_date), 'max_nuclei')

    if Yth~= 1;
       
    %% Segment DAPI image
    fileDAPI = ['nuclei_' exp_name  num2str(counter) '.mat'];
    if exist([outfile_prefix_seg fileDAPI],'file');
       load([outfile_prefix_seg fileDAPI]);   % load the existing file
    else;
       [Label_low,Label_mid,Label_hi,nuclei] = B1_autosegment_nuclei5(DAPI_ims,dapi_label,Yim,dapi_threshold,min_nucleus_size,max_nucleus_size); % Segment DAPI images in 3D with three different thresholds
       save([outfile_prefix_seg fileDAPI],'Label_low','Label_mid','Label_hi','nuclei','-mat','-v7.3');     % Save the segmented nuclei      
    end;

    %% Segment TRANS image
    fileTRANS = ['Lab_' exp_name  num2str(counter) '.mat'];
