function [out1,out2,out3] = ginput_color(arg1, color_rgb)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig, color_rgb);
...
function initialState = setupFcn(fig,color_rgb)
...
initialState.CrossHair = createCrossHair(fig,color_rgb);
...
function updateCrossHair(fig, crossHair, color_rgb)
...
function crossHair = createCrossHair(fig,color_rgb)
...
for k = 1:4
    crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', color_rgb, 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
end
end
