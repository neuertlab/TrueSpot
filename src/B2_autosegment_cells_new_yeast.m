function [Lab,plane,CellInfo,trans_plane,CellImage] = B2_autosegment_cells_new(TRANS_ims,nuclei,auto_mode,Yim,min_cell_size,max_cell_size,dapi_label);
...
elseif strcmp(auto_mode,'last5')
    plane = Z;
    start_plane = Z-4;                              % consider 3 planes above the focus plane to see the cell boundary
                                                    %Changed to 9 for
                                                    %mammalian cells BK
                                                    %4/7/16
    end_plane = Z;                                % consider 7 planes above the focus plane to see the cell boundary
    if end_plane> size(TRANS_ims,3)
        end_plane = size(TRANS_ims,3);
    end
    if start_plane > size(TRANS_ims,3)
    ...
    trans_ring_planes = TRANS_ims(:,:,...
                           start_plane:end_plane);
    trans_plane = max(trans_ring_planes,[],3);            % Maximum intensity z-projection
    trans1 = trans_plane; %imopen(trans_plane,strel('disk',5)); % 25 %Added Gaussian smoothing
if Yim == 1;
        figure(52); clf; imshow(trans1,[]);   title('last5');   impixelinfo; imwrite(uint16(trans1),['trans1.tif']);% comment out for analysis
else;
end;
    trans2 = imopen(trans_plane,strel('disk',40)); % 25                                   % smooth YFP image with a disk filter of 50px to generate an background image
if Yim == 1;
        figure(53); clf; imshow(trans2,[]);impixelinfo; imwrite(uint16(trans2),['trans2.tif']);
else;
end;
...
if Yim == 1;
        figure(99); clf; imshow(trans2,[]);impixelinfo; imwrite(uint16(trans2b),['trans2b.tif']);
else;
end;
...
if Yim == 1;
    figure(59); clf; imshow(Label1,[]); impixelinfo; imwrite(uint16(Label1),['Label1.tif']);
else;
end;
m = max(Label1(:))                                                          % determine the maximum number of segmented cells 

%% Go through each cell and analyze the shape, romove cells and get information
%Label1 = Label
...
for j = 1 : mm;                                                              % organise cell information
    ...
    try CellInfo(j,[1:2]) = L4.Centroid; catch  CellInfo(j,[1:2]) =[NaN,NaN];  end;                               % Cell Center
