    ...
    for j = 1:max_tps
        ...
        for k = 1:max_imnum
            try
                ...
                load(filename,['PARcy5_' nucTH]);
                        ...
                        for p = 1:size(PARcy5_low.xinit,2)
                            ...
                            for pp = 1:size(PARcy5_low.xinit,2)    %find distance squared
                                ...
                                dists(1,pp,3) = ((PARcy5_low.zinit(m,p)- PARcy5_low.zinit(m,pp))*3)^2;
                            end
                ...
                All_non_cloud(1:size(PARcy5_low.TotExpInt,1),counter3:counter3-3+size(PARcy5_low.TotExpInt,2)) = PARcy5_low.TotExpInt(:,3:end);          %RNAs without the cloud
                All_non_cloud_tp(counter5:counter5+size(PARcy5_low.TotExpInt,1)-1,1:size(PARcy5_low.TotExpInt,2)-2,counter) = PARcy5_low.TotExpInt(:,3:end); 
                All_cloud(1:size(PARcy5_low.TotExpInt,1),counter4) = PARcy5_low.TotExpInt(:,1);
                All_cloud(size(PARcy5_low.TotExpInt,1)+1:size(PARcy5_low.TotExpInt,1)*2,counter4) = PARcy5_low.TotExpInt(:,2);
                ...
                if size(PARcy5_low.TotExpInt,2) >1 & size(find(PARcy5_low.TotExpInt(:,3:end) <6000),1) > 0  %If there is a cell with more than one RNA spot and they are less than 4000
                    non_cloud = PARcy5_low.TotExpInt(:,3:end);          %RNAs without the cloud
                    RNA_int = nanmedian(non_cloud(find(PARcy5_low.TotExpInt(:,3:end) <6000)))  %RNA intensity for one spot is median of non-clod spots under 4000 intensity
                else
                ...
                for m = 1:size(PARcy5_low.TotExpInt,1)      %loop for number of cells in image
                    if PARcy5_low.TotExpInt(m,1) > 10*RNA_int
                        ...
                        temp_trans_numb(m,counter1) = PARcy5_low.TotExpInt(m,1)/RNA_int+size(find(PARcy5_low.TotExpInt(m,3:end)<6000),2);
                       % temp_trans_fl(m,counter1) = PARcy5_low.TotExpInt(m,3:end)
                    else
                        temp_trans_numb(m,counter1) = size(find(PARcy5_low.TotExpInt(m,:)<6000),2);
                    end
                end
                counter1 = counter1+1;
