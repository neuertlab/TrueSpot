...
%6: image number
%7: Used for bins in boxplots (on A0 Analysis page)
...
% nucTH = 'low'
%% Do analysis
perform_cell_functions = 1; %This is for determining if something is in a cloud or not, and must load the cell boundaries (which take time)
perform_nuc_functions = 1; %This is for determining the distance to the nucleus, and it must load the nucleus info, which takes time to load
...
% AF594_cloudInt
z_adjust = 0;   %This adjusts the multiplier applied to the z axis to account for the fact that distance between slices will be different that distances between neighboring pixels
...
CY5_AF594_TMR = zeros(2,3);         %matrix. Each row corresponds to a single cell, first column is how many CY5 (Xist), second column is how many AF594 (Tsix), third column is how many TMR (Jpx)
...
CY5_Nuc_dist = zeros(2,3); CY5_Nuc_dist_tp = {};
TMR_Nuc_dist = zeros(2,3); TMR_Nuc_dist_tp = {};
Cl_Nuc_dist = zeros(2,3); Cl_Nuc_dist_tp = {};
C1_CY5_dist = zeros(2,3); C1_CY5_dist_tp = {}; 
C2_CY5_dist = zeros(2,3); C2_CY5_dist_tp = {}; 
C1_TMR_dist = zeros(2,3); C1_TMR_dist_tp = {}; 
C2_TMR_dist = zeros(2,3); C2_TMR_dist_tp = {};
if isempty(CY5_name)
    thNum_CY5 = [];
end
...
for i = 1: size(unique(times1),2)
    CY5_AF594_TMR_tp{i} = zeros(1,2); %Same as CY5_AF594_TMR, but separated by timepoint (first dimension of cell is timepoint)
end
    ...
    for j = 1:max_tps
        ...
        for k = 1:max_imnum
            try
                ...
                fileLAB = char(['Lab_' dates0{i} '_' times0{i}{j} '_F1-2-1'  CY5_name{i}(1:size(CY5_name{i},2)-3) '-' TMR_name{i}(2:size(CY5_name{i},2)-3) '_img' num2str(k) '.mat']);
                load(fileLAB,'cells'); %'NucInfo','CellInfo',
                Lab = cells;
                end
                if perform_nuc_functions
                fileNUC = char(['nuclei_' dates0{i} '_' times0{i}{j} '_F1-2-1'  CY5_name{i}(1:size(CY5_name{i},2)-3) '-' TMR_name{i}(2:size(CY5_name{i},2)-3) '_img' num2str(k) '.mat']);
                load(fileNUC,'Label_low'); %'NucInfo','CellInfo',
                end
                if not(isempty(TMR_name)) & TMR_hascloud & perform_cell_functions   %Load TMR clouds
                    filename = ['mRNA_' dates0{i} '_F1-2-1' TMR_name{i} num2str(thTMRall{i}(thNum_TMR)) '_' num2str(thAF594all{i}(thNum_TMR)) CY5_name{i} num2str(thCY5all{i}(thNum_TMR)) '_' times0{i}{j} '_im' num2str(k) '_stringent_cloud'];
                    % AF594_name{i} num2str(thAF594all{i}(thNum_TMR))
                    load(filename,['PARtmr_' nucTH]);
                end
                if not(isempty(CY5_name))
                    filename = ['mRNA_' dates0{i} '_F1-2-1' TMR_name{i} num2str(thTMRall{i}(thNum_CY5)) '_' num2str(thAF594all{i}(thNum_CY5)) CY5_name{i} num2str(thCY5all{i}(thNum_CY5)) '_' times0{i}{j} '_im' num2str(k) '_stringent_cloud'];
                    %AF594_name{i} num2str(thAF594all{i}(thNum_CY5))
                    load(filename,['PARcy5_' nucTH]);
                    %figure(2); clf; imshow(max(PARcy5_low.clouds1,[],3),[])
                    for m = 1: size(PARcy5_low.TotExpInt,1) %Go through every cell in this image
                        CY5_AF594_TMR(counter9,5) = str2num(dates0{i})/6;  %5: date of experiment
                        CY5_AF594_TMR_tp{counter11}(counter10,5) = str2num(dates0{i})/6; %5: date of experiment
                        ...
                        if X1 > A1; X1 = A1; end
                        if Y1 > A2; Y1 = A2; end
                        Label_low_cell = Label_low(Y0:Y1,X0:X1,:);  %Get nucleus for just the cell
                        if CY5_hascloud | TMR_hascloud
                            if CY5_hascloud
                                CY5_cloud1_cell = PARcy5_low.clouds1(Y0:Y1,X0:X1,:);      %Generate just CY5 cloud 1 for this cell
                                CY5_cloud2_cell = PARcy5_low.clouds2(Y0:Y1,X0:X1,:);      %Generate CY5 cloud 2 for this cell
                            end
                            if not(isempty(TMR_name)) & TMR_hascloud
                                TMR_cloud1_cell = PARtmr_low.clouds1(Y0:Y1,X0:X1,:);      %Generate TMR cloud 1 for this cell
                                TMR_cloud2_cell = PARtmr_low.clouds2(Y0:Y1,X0:X1,:);      %Generate TMR cloud 2 for this cell
                            end
                        end
                        ...
                        for p = 1:size(PARcy5_low.xinit,2)
                            same_pos = find(PARcy5_low.xinit(m,:) == PARcy5_low.xinit(m,p));
                            if size(same_pos,2)>10
                                for q = 1:size(same_pos,2)
                                    PARcy5_low.xinit(m,same_pos(q)) = NaN;
                                    PARcy5_low.yinit(m,same_pos(q)) = NaN;
                                    PARcy5_low.zinit(m,same_pos(q)) = NaN;
                                    PARcy5_low.TotExpInt(m,same_pos(q)) = NaN;
                                end
                            end
                        end
                        ...
                        for p = 1:size(PARcy5_low.xinit,2)
                            dists = zeros(1,size(size(PARcy5_low.xinit,2),3)); %stores distance from transcript to others
                            if not(isnan(PARcy5_low.xinit(m,p)))
                                for pp = 1:size(PARcy5_low.xinit,2)    %find distance squared
                                    dists(1,pp,1) = (PARcy5_low.xinit(m,p)- PARcy5_low.xinit(m,pp))^2;
                                    dists(1,pp,2) = (PARcy5_low.yinit(m,p)- PARcy5_low.yinit(m,pp))^2;
                                    dists(1,pp,3) = ((PARcy5_low.zinit(m,p)- PARcy5_low.zinit(m,pp))*z_adjust)^2;
                                end
                                distF = sqrt(dists(1,:,1)+dists(1,:,2)+dists(1,:,3));
                                for pp = 1:size(PARcy5_low.xinit,2)
                                    if p ~= pp & distF(pp) < same_dist
                                        PARcy5_low.xinit(m,pp) = NaN;
                                        PARcy5_low.yinit(m,pp) = NaN;
                                        PARcy5_low.zinit(m,pp) = NaN;
                                        PARcy5_low.TotExpInt(m,pp) = NaN;
                                    end
                                end
                            end
                        end
                        ...
                        %%%positions)
                        CY5_pos_init(counter9,1:size(PARcy5_low.xinit,2),1) = PARcy5_low.xinit(m,1:size(PARcy5_low.xinit,2));
                        CY5_pos_init(counter9,1:size(PARcy5_low.xinit,2),2) = PARcy5_low.yinit(m,1:size(PARcy5_low.yinit,2));
                        CY5_pos_init(counter9,1:size(PARcy5_low.xinit,2),3) = PARcy5_low.zinit(m,1:size(PARcy5_low.zinit,2));
                        CY5_pos_init_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2),1) = PARcy5_low.xinit(m,1:size(PARcy5_low.xinit,2));
                        CY5_pos_init_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2),2) = PARcy5_low.yinit(m,1:size(PARcy5_low.yinit,2));
                        CY5_pos_init_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2),3) = PARcy5_low.zinit(m,1:size(PARcy5_low.zinit,2));
                        %% Determine distance to edge of nucleus
                        if perform_nuc_functions
                        NucBorder1 = bwmorph3(Label_low_cell,'remove');
                        %                         figure(300); clf;
                        ...
                        %                             pause(.5)
                        %                         end
                        lin_Nuc = find(NucBorder1 == 1);[xNuc,yNuc,zNuc] = ind2sub(size(NucBorder1),lin_Nuc(1:3:end)); clear lin_Nuc; %These are vertical vectors (x differs)
                        pNuc1 = xNuc; pNuc1(:,:,2) = yNuc; pNuc1(:,:,3) = zNuc; clear xNuc yNuc zNuc NucBorder1; %combine matrices
                        pNuc1 = int16(pNuc1);                        
                        pNuc2 = repmat(pNuc1,[1,size(PARcy5_low.xinit,2),1]);  %replicate in y axis the matricies
                        pTran = PARcy5_low.xinit(m,1:size(PARcy5_low.xinit,2));
                        pTran(:,:,2) = PARcy5_low.yinit(m,1:size(PARcy5_low.yinit,2));
                        pTran(:,:,3) = PARcy5_low.zinit(m,1:size(PARcy5_low.zinit,2));
                        pTran = int16(pTran);
                        ...
                        CY5_Nuc_dist(counter9,1:size(PARcy5_low.xinit,2)) = min(pDiff,[],1);
                        CY5_Nuc_dist_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2)) = min(pDiff,[],1);
                        clear pDiff pNuc2
                        if CY5_hascloud
                            if not(isnan(PARcy5_low.xinit(m,1)))
                                ...
                                if size(pC1,2) < 1
                                    ...
                                    C1_CY5_dist(counter9,1:size(PARcy5_low.xinit,2)) = nan(1,size(PARcy5_low.xinit,2));
                                    C1_CY5_dist_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2)) = nan(1,size(PARcy5_low.xinit,2));                                      
                                else
                                    ...
                                    pDiff(:,:,3) = pDiff(:,:,3)*z_adjust;   %Adjust distance determined by z axis based on how large
                                    pDiff = sum(pDiff.^2,3); %pDiff = pDiff.^.5; 
                                    C1_CY5_dist(counter9,1:size(PARcy5_low.xinit,2)) = min(pDiff,[],2)';
                                    C1_CY5_dist_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2)) = min(pDiff,[],2)';
                                    clear pDiff
                                end
                                %%%
...
%                                 end
                            ...
                            if not(isnan(PARcy5_low.xinit(m,2)))               %Do the same, but for the second cloud
                                ...
                                if size(pC1,2) < 1
                                    Cl_Nuc_dist(counter9,2) = NaN;
                                    Cl_Nuc_dist_tp{counter11}(counter10,2) = NaN;
                                    C2_CY5_dist(counter9,1:size(PARcy5_low.xinit,2)) = nan(1,size(PARcy5_low.xinit,2));
                                    C2_CY5_dist_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2)) = nan(1,size(PARcy5_low.xinit,2));                                     
                                else
                                    pC1_2 = repmat(pC1,[size(pNuc1,1),1,1]); %make copies for comparison. Commented outfor memory
                                    pNuc2 = repmat(pNuc1,[1,size(pC1,2),1]);        %Make copies for comparison
                                    pDiff = pNuc2-pC1_2; clear pNuc2 pC1_2; pDiff = int32(pDiff);
                                    ...
                                    pDiff(:,:,3) = pDiff(:,:,3)*z_adjust;   %Adjust distance determined by z axis based on how large
                                    pDiff = sum(pDiff.^2,3); %pDiff = pDiff.^.5;
                                    C2_CY5_dist(counter9,1:size(PARcy5_low.xinit,2)) = min(pDiff,[],2)';
                                    C2_CY5_dist_tp{counter11}(counter10,1:size(PARcy5_low.xinit,2)) = min(pDiff,[],2)';                                    
                                    clear pDiff
                                end
                                %%%
                                %%% Slow but memory-efficient way
...
%                                 end
                        ...
                        %TMR_C2_dist_tp = Distance of each TMR transcript from the 2nd cloud (arranged like CY5_AF594_TMR_tp)
                        %%%
                        transc_num = size(find(PARcy5_low.xinit(m,:)>0),2);
                        CY5_int(counter9,3:size(PARcy5_low.TotExpInt,2)) = PARcy5_low.TotExpInt(m,3:size(PARcy5_low.TotExpInt,2));  %Store total experimental intensity
                        CY5_int_tp{counter11}(counter10,3:size(PARcy5_low.TotExpInt,2)) = PARcy5_low.TotExpInt(m,3:size(PARcy5_low.TotExpInt,2)); %Store total experimental intensity
                        ...
                        % CY5_AF594_TMR(counter9,1) = nansum([nansum(PARcy5_low.cytoRNA(m,:)),nansum(PARcy5_low.nucRNA(m))]); %Set Jpx value to nuclear + Cytoplasmic RNA molecules
                        %CY5_AF594_TMR_tp{counter11}(counter10,1) = nansum([nansum(PARcy5_low.cytoRNA(m,:)),nansum(PARcy5_low.nucRNA(m))]); %Set Jpx value to nuclear + Cytoplasmic RNA molecules
                        ...
                        if perform_cell_functions
                        if CY5_hascloud
                            for temp = 1:size(PARcy5_low.xinit,2)
                        ...
                        %%%
                        if CY5_hascloud && not(isnan(PARcy5_low.TotExpInt(m,1))) && not(isnan(PARcy5_low.TotExpInt(m,2)))
                            CY5_AF594_TMR(counter9,1) = CY5_AF594_TMR(counter9,1)-2 + PARcy5_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction+ PARcy5_low.TotExpInt(m,2)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,1) =CY5_AF594_TMR_tp{counter11}(counter10,1) - 2 + PARcy5_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction + PARcy5_low.TotExpInt(m,2)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR(counter9,8) = sum(PARcy5_low.TotExpInt(m,1:2) > CY5_tran_thres*CY5_cloudInt(i)*cloud_correction)+sum(PARcy5_low.TotExpInt(m,3:size(PARcy5_low.TotExpInt,2)) > CY5_tran_thres*CY5_cloudInt(i));        %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR_tp{counter11}(counter10,8) =CY5_AF594_TMR(counter9,8);      %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR(counter9,11) = PARcy5_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction;  %Number of transcripts inside the first cloud
                            CY5_AF594_TMR(counter9,12) = PARcy5_low.TotExpInt(m,2)/CY5_cloudInt(i)/cloud_correction; %Number of transcripts inside the second cloud
                        ...
                        elseif CY5_hascloud && not(isnan(PARcy5_low.TotExpInt(m,1)))
                            CY5_AF594_TMR(counter9,1) = CY5_AF594_TMR(counter9,1)-1 + PARcy5_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,1) =CY5_AF594_TMR_tp{counter11}(counter10,1) - 1 + PARcy5_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR(counter9,8) = sum(PARcy5_low.TotExpInt(m,1) > CY5_tran_thres*CY5_cloudInt(i)*cloud_correction)+sum(PARcy5_low.TotExpInt(m,3:size(PARcy5_low.TotExpInt,2)) > CY5_tran_thres*CY5_cloudInt(i));        %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR_tp{counter11}(counter10,8) =CY5_AF594_TMR(counter9,8);      %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR(counter9,11) = PARcy5_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,11) = CY5_AF594_TMR(counter9,11);
                        else
                            CY5_AF594_TMR(counter9,8) = sum(PARcy5_low.TotExpInt(m,:) > CY5_tran_thres*CY5_cloudInt(i));        %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR_tp{counter11}(counter10,8) =  CY5_AF594_TMR(counter9,8);        %Number of transcription sites/clouds in the cell
                        end
                ...
                if not(isempty(AF594_name))
                    filename = ['mRNA_' dates0{i} '_F1-2-1' TMR_name{i} num2str(thTMRall{i}(thNum_AF594)) AF594_name{i} num2str(thAF594all{i}(thNum_AF594)) CY5_name{i} num2str(thCY5all{i}(thNum_AF594)) '_' times0{i}{j} '_im' num2str(k) '_stringent_cloud'];
                    load(filename,['PARaf594_' nucTH]);
                    for m = 1: size(PARaf594_low.TotExpInt,1) %Go through every cell in this image
                        %%%Eliminate dead pixels  (needs to be improved)
                        for p = 1:size(PARaf594_low.xinit,2)
                            same_pos = find(PARaf594_low.xinit(m,:) == PARaf594_low.xinit(m,p));
                            if size(same_pos,2)>10
                                for q = 1:size(same_pos,2)
                                    PARaf594_low.xinit(m,same_pos(q)) = NaN;
                                end
                            end
                        end
                        %%% Below removes duplicates of the same
                        ...
                        %%% transcripts, and eliminates others
                        for p = 1:size(PARaf594_low.xinit,2)
                            dists = zeros(1,size(size(PARaf594_low.xinit,2),3)); %stores distance from transcript to others
                            if not(isnan(PARcy5_low.xinit(m,p)))
                                for pp = 1:size(PARaf594_low.xinit,2)    %find distance squared
                                    dists(1,pp,1) = (PARaf594_low.xinit(m,p)- PARaf594_low.xinit(m,pp))^2;
                                    dists(1,pp,2) = (PARaf594_low.yinit(m,p)- PARaf594_low.yinit(m,pp))^2;
                                    dists(1,pp,3) = ((PARaf594_low.zinit(m,p)- PARaf594_low.zinit(m,pp))*3)^2;
                                end
                                distF = sqrt(dists(1,:,1)+dists(1,:,2)+dists(1,:,3));
                                for pp = 1:size(PARaf594_low.xinit,2)
                                    if p ~= pp & distF(pp) < same_dist
                                        PARaf594_low.xinit(m,pp) = NaN;
                                        PARaf594_low.yinit(m,pp) = NaN;
                                        PARaf594_low.zinit(m,pp) = NaN;
                                        PARaf594_low.TotExpInt(m,pp) = NaN;
                                    end
                                end
                            end
                        end
                        %%%
                        transc_num = size(find(PARaf594_low.xinit(m,:)>0),2);
                        ...
                        %                         CY5_AF594_TMR(counter9,2) = nansum([nansum(PARaf594_low.cytoRNA(m,:)),nansum(PARaf594_low.nucRNA(m))]); %Set Jpx value to nuclear + Cytoplasmic RNA molecules
                        %                         CY5_AF594_TMR_tp{counter11}(counter10,2) = nansum([nansum(PARaf594_low.cytoRNA(m,:)),nansum(PARaf594_low.nucRNA(m))]); %Set Jpx value to nuclear + Cytoplasmic RNA molecules
                        if AF594_hascloud & not(isnan(PARaf594_low.TotExpIntCloud1(m,1))) & not(isnan(PARaf594_low.TotExpIntCloud1(m,2)))
                            CY5_AF594_TMR(counter9,2) = CY5_AF594_TMR(counter9,2)-2 + PARaf594_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction+ PARaf594_low.TotExpInt(m,2)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,2) = CY5_AF594_TMR_tp{counter11}(counter10,2) - 2 + PARaf594_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction + PARaf594_low.TotExpInt(m,2)/CY5_cloudInt(i)/cloud_correction;
                        elseif AF594_hascloud & not(isnan(PARaf594_low.TotExpIntCloud1(m,1)))
                            CY5_AF594_TMR(counter9,2) = CY5_AF594_TMR(counter9,2)-1 + PARaf594_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,2) = CY5_AF594_TMR_tp{counter11}(counter10,2) - 1 + PARaf594_low.TotExpInt(m,1)/CY5_cloudInt(i)/cloud_correction;
                        end
                        CY5_AF594_TMR(counter9,4) = times1(counter11); %stores timepoint
                        CY5_AF594_TMR(counter9,5) = str2num(dates0{i})*.000001; %stores date of experiment
                        CY5_AF594_TMR(counter9,6) = k; %stores image number
                ...
                if not(isempty(TMR_name))
                    ...
                    for m = 1: size(PARtmr_low.TotExpInt,1) %Go through every cell in this image
                        ...
                        pNuc1 = xNuc; pNuc1(:,:,2) = yNuc; pNuc1(:,:,3) = zNuc; clear xNuc yNuc zNuc NucBorder1; %combine matrices
                        pNuc1 = int16(pNuc1); 
                        pNuc2 = repmat(pNuc1,[1,size(PARtmr_low.xinit,2),1]);  %replicate in y axis the matricies
                        %pNuc2 = int16(pNuc2);
                        pTran = PARtmr_low.xinit(m,1:size(PARtmr_low.xinit,2));
                        pTran(:,:,2) = PARtmr_low.yinit(m,1:size(PARtmr_low.yinit,2));
                        pTran(:,:,3) = PARtmr_low.zinit(m,1:size(PARtmr_low.zinit,2));
                        pTran = int16(pTran);
                        pTran1 = repmat(pTran,[size(pNuc2,1),1,1]);
                        pDiff = pNuc2-pTran1; clear pNuc2 pTran1; pDiff = int32(pDiff);
                        pDiff(:,:,3) = pDiff(:,:,3)*z_adjust;   %Adjust distance determined by z axis based on how large
                        pDiff = sum(pDiff.^2,3); %pDiff = pDiff.^.5; %To keep in in integer form, do not square root
                        TMR_Nuc_dist(counter9,1:size(PARtmr_low.xinit,2)) = min(pDiff,[],1);
                        TMR_Nuc_dist_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2)) = min(pDiff,[],1);
                        clear pDiff
                        end
                        %%
                        %%%store the positions of all transcripts
                        TMR_pos_init(counter9,1:size(PARtmr_low.xinit,2),1) = PARtmr_low.xinit(m,1:size(PARtmr_low.xinit,2));
                        TMR_pos_init(counter9,1:size(PARtmr_low.xinit,2),2) = PARtmr_low.yinit(m,1:size(PARtmr_low.yinit,2));
                        TMR_pos_init(counter9,1:size(PARtmr_low.xinit,2),3) = PARtmr_low.zinit(m,1:size(PARtmr_low.zinit,2));
                        TMR_pos_init_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2),1) = PARtmr_low.xinit(m,1:size(PARtmr_low.xinit,2));
                        TMR_pos_init_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2),2) = PARtmr_low.yinit(m,1:size(PARtmr_low.yinit,2));
                        TMR_pos_init_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2),3) = PARtmr_low.zinit(m,1:size(PARtmr_low.zinit,2));
                        %%% Determine transcript number
                        transc_num = size(find(PARtmr_low.xinit(m,:)>0),2);
                        ...
                        TMR_int(counter9,3:size(PARtmr_low.TotExpInt,2)) = PARtmr_low.TotExpInt(m,3:size(PARtmr_low.TotExpInt,2));  %Store total experimental intensity
                        TMR_int_tp{counter11}(counter10,3:size(PARtmr_low.TotExpInt,2)) = PARtmr_low.TotExpInt(m,3:size(PARtmr_low.TotExpInt,2)); %Store total experimental intensity
                        %CY5_AF594_TMR(counter9,3) = nansum([nansum(PARtmr_low.cytoRNA(m,:)),nansum(PARtmr_low.nucRNA(m))]); %Set Jpx value to nuclear + Cytoplasmic RNA molecules
                        %                         CY5_AF594_TMR_tp{counter11}(counter10,3) = nansum([nansum(PARtmr_low.cytoRNA(m,:)),nansum(PARtmr_low.nucRNA(m))]); %Set Jpx value to nuclear + Cytoplasmic RNA molecules
                        %%% Determine whether transcripts are inside clouds
                        if perform_cell_functions
                        if CY5_hascloud
                            for temp = 1:size(PARtmr_low.xinit,2)
...
%                                 xC1 = xC1'; yC1 = yC1'; zC1 = zC1';     %Change x's to y's
                                ...
                                %%% fast but memory-intensive way of determining distances
                                if size(pC1,1) < 1
                                    C1_TMR_dist(counter9,1:size(PARtmr_low.xinit,2)) = nan(1,size(PARtmr_low.xinit,2));
                                    C1_TMR_dist_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2)) = nan(1,size(PARtmr_low.xinit,2));
                                else
                                    ...
                                    pDiff(:,:,3) = pDiff(:,:,3)*z_adjust;   %Adjust distance determined by z axis based on how large
                                    pDiff = sum(pDiff.^2,3); %pDiff = pDiff.^.5;
                                    C1_TMR_dist(counter9,1:size(PARtmr_low.xinit,2)) = min(pDiff,[],1);
                                    C1_TMR_dist_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2)) = min(pDiff,[],1);
                                    clear pDiff
                                end
                                %%%
                                %%% Slow but memory-efficient way
...
%                                 xC1 = xC1'; yC1 = yC1'; zC1 = zC1';     %Change x's to y's
                                ...
                                %%% fast but memory-intensive way of determining distances
                                if size(pC1,1) < 1
                                    C2_TMR_dist(counter9,1:size(PARtmr_low.xinit,2)) = nan(1,size(PARtmr_low.xinit,2));
                                    C2_TMR_dist_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2)) = nan(1,size(PARtmr_low.xinit,2));
                                else
                                    pTran1 = repmat(pTran,[size(pC1,1),1,1]);
                                    pC1_2 = repmat(pC1,[1,size(pTran1,2),1]); %make copies for comparison. Commented outfor memory
                                    pDiff = pTran1-pC1_2; clear pC1_2 pTran1; pDiff = int32(pDiff);
                                    pDiff(:,:,3) = pDiff(:,:,3)*z_adjust;   %Adjust distance determined by z axis based on how large
                                    pDiff = sum(pDiff.^2,3); %pDiff = pDiff.^.5;
                                    C2_TMR_dist(counter9,1:size(PARtmr_low.xinit,2)) = min(pDiff,[],1);
                                    C2_TMR_dist_tp{counter11}(counter10,1:size(PARtmr_low.xinit,2)) = min(pDiff,[],1);
                                    clear pDiff
                                end
                                %%%
                                %%% Slow but memory-efficient way
...
%                                 end
                        ...
                        %%%
                        if TMR_hascloud & not(isnan(PARtmr_low.TotExpInt(m,1))) & not(isnan(PARtmr_low.TotExpInt(m,2)))
                            CY5_AF594_TMR(counter9,3) = CY5_AF594_TMR(counter9,3)-2 + PARtmr_low.TotExpInt(m,1)/TMR_cloudInt(i)/cloud_correction+ PARtmr_low.TotExpInt(m,2)/TMR_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,3) = CY5_AF594_TMR_tp{counter11}(counter10,3) - 2 + PARtmr_low.TotExpInt(m,1)/TMR_cloudInt(i)/cloud_correction + PARtmr_low.TotExpInt(m,2)/TMR_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR(counter9,10) = sum(PARtmr_low.TotExpInt(m,1:2) > TMR_tran_thres*TMR_cloudInt(i)*cloud_correction)+sum(PARtmr_low.TotExpInt(m,3:size(PARtmr_low.TotExpInt,2)) > TMR_tran_thres*TMR_cloudInt(i));        %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR_tp{counter11}(counter10,10) =CY5_AF594_TMR(counter9,10);      %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR(counter9,13) = PARtmr_low.TotExpInt(m,1)/TMR_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR(counter9,14) = PARtmr_low.TotExpInt(m,2)/TMR_cloudInt(i)/cloud_correction;
                        ...
                        elseif TMR_hascloud & not(isnan(PARtmr_low.TotExpIntCloud1(m,1)))
                            CY5_AF594_TMR(counter9,3) = CY5_AF594_TMR(counter9,3)-1 + PARtmr_low.TotExpInt(m,1)/TMR_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,3) = CY5_AF594_TMR_tp{counter11}(counter10,3) - 1 + PARtmr_low.TotExpInt(m,1)/TMR_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR(counter9,10) = sum(PARtmr_low.TotExpInt(m,1) > TMR_tran_thres*TMR_cloudInt(i)*cloud_correction)+sum(PARtmr_low.TotExpInt(m,3:size(PARtmr_low.TotExpInt,2)) > TMR_tran_thres*TMR_cloudInt(i));        %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR_tp{counter11}(counter10,10) =CY5_AF594_TMR(counter9,10);      %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR(counter9,13) = PARtmr_low.TotExpInt(m,1)/TMR_cloudInt(i)/cloud_correction;
                            CY5_AF594_TMR_tp{counter11}(counter10,13) = CY5_AF594_TMR(counter9,13);
                        else
                            CY5_AF594_TMR(counter9,10) = sum(PARtmr_low.TotExpInt(m,:) > TMR_tran_thres*TMR_cloudInt(i));        %Number of transcription sites/clouds in the cell
                            CY5_AF594_TMR_tp{counter11}(counter10,10) =  CY5_AF594_TMR(counter9,10);        %Number of transcription sites/clouds in the cell
                        end
                        CY5_AF594_TMR(counter9,4) = times1(counter11);  %stores timepoint
                        CY5_AF594_TMR(counter9,5) = str2num(dates0{i})*.000001; %stores date of experiment
                        ...
                        for p = 1:size(PARtmr_low.xinit,2)
                            ...
                            if not(isnan(PARtmr_low.xinit(m,p)))
                                for pp = 1:size(PARcy5_low.xinit,2)    %find distance squared
                                    dists(1,pp,1) = (PARtmr_low.xinit(m,p)- PARcy5_low.xinit(m,pp))^2;
                                    dists(1,pp,2) = (PARtmr_low.yinit(m,p)- PARcy5_low.yinit(m,pp))^2;
                                    dists(1,pp,3) = ((PARtmr_low.zinit(m,p)- PARcy5_low.zinit(m,pp))*z_adjust)^2;
                                end
                                distF = sqrt(dists(1,:,1)+dists(1,:,2)+dists(1,:,3));
                        ...
                        for p = 1:size(PARtmr_low.xinit,2)
                            ...
                            if not(isnan(PARtmr_low.xinit(m,p)))
                                for pp = 1:size(PARtmr_low.xinit,2)    %find distance squared
                                    dists(1,pp,1) = (PARtmr_low.xinit(m,p)- PARtmr_low.xinit(m,pp))^2;
                                    dists(1,pp,2) = (PARtmr_low.yinit(m,p)- PARtmr_low.yinit(m,pp))^2;
                                    dists(1,pp,3) = ((PARtmr_low.zinit(m,p)- PARtmr_low.zinit(m,pp))*z_adjust)^2;
                                end
                                distF = sqrt(dists(1,:,1)+dists(1,:,2)+dists(1,:,3));
                                if size(find(distF < dist_col),2) > 0
                    ...
                    if not(isempty(CY5_name))
                        counter9 = counter9_int;
                        counter10 = counter10_int;
                        for m = 1: size(PARcy5_low.TotExpInt,1)
                            for p = 1:size(PARcy5_low.xinit,2)
                                dists = zeros(1,size(size(PARcy5_low.xinit,2),3)); %stores distance from transcript to others
                                if not(isnan(PARcy5_low.xinit(m,p)))
                                    for pp = 1:size(PARtmr_low.xinit,2)    %find distance squared
                                        dists(1,pp,1) = (PARcy5_low.xinit(m,p)- PARtmr_low.xinit(m,pp))^2;
                                        dists(1,pp,2) = (PARcy5_low.yinit(m,p)- PARtmr_low.yinit(m,pp))^2;
                                        dists(1,pp,3) = ((PARcy5_low.zinit(m,p)- PARtmr_low.zinit(m,pp))*z_adjust)^2;
                                    end
                                    distF = sqrt(dists(1,:,1)+dists(1,:,2)+dists(1,:,3));
                                    if size(find(distF < dist_col),2) > 0
